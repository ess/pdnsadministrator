-------------------------------
== PDNS-Admin ==
-------------------------------

=== Installation ===
 1. Upload all files, overwriting any existing files

 2. If your server allows it, CHMOD the following files and directories to 755
    or, if you have problems, 777.

    ./skins/
    ./packages/
    ./settings.php

 3. Go to the install directory in your browser
 4. Follow the instructions
 5. Delete the install directory

=== Upgrading ===

 Upgrading is done in the same way as installing. Make sure you select the
 "upgrade" option. Before upgrading, make a note of your current PDNS-Admin
 installation version: you will need it to correctly upgrade. This information
 should be available from the AdminCP of your existing install.

 It is recommended you keep old settings.php. Do not overwrite it as you do
 with other files

=== Requirements ===

 PDNS-Admin works with PHP 5.3.0 or higher. This is necessary due to the IPv6 support.
 MySQL 3.22 or higher is required. PDNS-Admin works on any operating
 system and web server combination.

 If you experience a problem and you meet these requirements, please report it
 as a bug. The latest versions of PHP and MySQL are recommended to keep
 PDNS-Admin running its fastest and safest.

=== License ===

 This software is released under the GNU General Public License (GPL). For the
 terms of the license, see the file docs/license.txt included in this
 distribution. The license is also available in many languages at
 http://www.gnu.org

=== How You Can Help ===

 If you know PHP and MySQL, and would like to help develop PDNS-Admin,
 please get in touch with Roger Libiez at the Google Code project page:
 https://code.google.com/p/pdnsadministrator/

 Feel free to send in feature requests, bugfixes, suggestions, or whatever.

=== Support ===

 If you're sure you've read the above information, and it simply will not work,
 please get in touch with the PDNS-Admin support community at:
 http://pdnsadmin.iguanadons.net/
